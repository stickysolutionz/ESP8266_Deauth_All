...
const char *IGNORE_LIST[] = {"AP1", "AP2", "AP3"};
const int IGNORE_LIST_SIZE = sizeof(IGNORE_LIST) / sizeof(IGNORE_LIST[0]);
...
// Check if an ESSID is in the ignore list
bool isIgnoredNetwork(const String &essid) {
    Serial.print("Checking network: "); Serial.println(essid);
    for (int i = 0; i < IGNORE_LIST_SIZE; ++i) {
        Serial.print("Against: "); Serial.println(IGNORE_LIST[i]);
        if (essid.equalsIgnoreCase(IGNORE_LIST[i])) {
            Serial.println("Ignored network found!");
            return true;
        }
    }
    Serial.println("Network not ignored.");
    return false;
}
...
void promiscCallback(uint8 *buf, uint16 len) {
    if (len >= 36) { // Adjust to match the minimum length of your sniffed packets
        struct sniffer_buf2 *sniffer = (struct sniffer_buf2 *)buf;
        if (sniffer->buf[0] == 0x80) { // Check for beacon packet
            uint8_t bssid[6];
            memcpy(bssid, sniffer->buf + 10, 6);
            int ssid_length = sniffer->buf[37];  // Ensure this is the correct offset for SSID length
            String essid = "";
            for (int i = 0; i < ssid_length; ++i) {
                essid.concat((char)sniffer->buf[i + 38]); // Ensure offset is correct
            }
            if (!isIgnoredNetwork(essid)) {
